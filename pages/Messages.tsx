...
import { MOCK_CONVERSATIONS, DataService } from '../services/dataService';
import { Message, Announcement, UserRole } from '../types';
import { Search, Bell, Mail, Send, CheckCheck, Tag, User as UserIcon, Plus, X } from 'lucide-react';
...
const Messages = () => {
  ...
  // Since we are mocking other users, we hardcode participant lookup for demo
  const getParticipantName = (id: string) => id === 'u3' ? 'Mr. Thompson' : 'Unknown';
  const getParticipantRole = (id: string) => id === 'u3' ? 'TEACHER' : 'USER';
  const getParticipantAvatar = (id: string) => `https://picsum.photos/200/200?random=${id}`;

  useEffect(() => {
    DataService.getAnnouncements().then(setAnnouncements);
  }, []);

  useEffect(() => {
    if (selectedConversationId) {
      DataService.getMessages(selectedConversationId).then(setMessages);
    }
  ...
  const handleSendMessage = async () => {
    ...
    await DataService.addMessage(selectedConversationId, newMessage);
  };
                     ...
                     >
                        <div className="flex justify-between items-start mb-1">
                           <div className="flex items-center gap-2">
                              <img src={getParticipantAvatar(conv.participantId)} className="w-8 h-8 rounded-full" alt="" />
                              <span className="font-semibold text-sm text-slate-800">{getParticipantName(conv.participantId)}</span>
                           </div>
                ...
                {announcements.map(ann => (
                  <div key={ann.id} className="p-4 hover:bg-white cursor-pointer transition-colors group">
                     <div className="flex justify-between items-start mb-1">
                        ...
                        <span className="text-[10px] text-slate-400">{ann.date ? new Date(ann.date).toLocaleDateString() : ''}</span>
                     </div>
               ...
               <div className="h-16 border-b border-slate-100 flex items-center px-6 justify-between">
                  <div className="flex items-center gap-3">
                     <div className="w-10 h-10 rounded-full bg-slate-200 overflow-hidden">
                        <img src={getParticipantAvatar('u3')} className="w-full h-full object-cover" alt="" />
                     </div>
                     <div>
                        <h3 className="font-bold text-slate-800">{getParticipantName('u3')}</h3>
                        <p className="text-xs text-slate-500">{getParticipantRole('u3')}</p>
                     </div>
                  </div>
               </div>
                  ...
                  <div key={ann.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                     ...
                     <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs text-slate-400 flex items-center gap-1">
                           <Bell size={12} /> {ann.date ? new Date(ann.date).toLocaleDateString() : ''}
                        </span>
                     </div>
